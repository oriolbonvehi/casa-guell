...
### Actualización 7 — 22 Sep 2026 (trilingüe EN, fix vídeo móvil, ortografía, +fotos +v9)
- **Versión INGLÉS (EN)** añadida: `content.js` bloque `en`; cada plato en `menuData.js` con name/desc {ca,es,en}; allergenLabels con `en`. Nombres propios sin traducir (Gilda, Cap i Pota, Puceta, La Bailona, Xuixo, Fricandó, Camarón, Idiazábal, Guillardeau). Descripciones para turistas en platos complejos (fricandó, cap i pota, canelones, xuixo, coulant, escudella, torrija...). `LanguageContext` cicla ca/es/en + `setLanguage(l)`; Header con selector CA/ES/EN (data-testid lang-ca/es/en + mobile-lang-*).
- **Ortografía corregida**: CA "d'avia"→"d'àvia" (ensaladilla, flam), "casquería"→"casqueria" (oreja), "probar"→"provar" (tagline filosofía), "Al·lergens"→"Al·lèrgens", solomillo CA "Filet", ES "Rovellones". Revisión CA/ES/EN sin faltas.
- **Fix vídeo móvil (bug)**: creadas versiones ligeras `/videos/mobile/v1..v9.mp4` (~100–650KB, 540px). Hero: `VideoSlot` con **doble búfer + crossfade** (precarga el siguiente clip). **Montaje condicional** con `useIsDesktop()` (matchMedia 768px): en móvil solo se monta el slot ligero (NO se cargan los vídeos pesados de escritorio); en escritorio solo el grid de 3 columnas. Sin audio, sin póster, carga instantánea.
